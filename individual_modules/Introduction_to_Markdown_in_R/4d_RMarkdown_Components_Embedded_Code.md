...
### Task
Since the last task, some further content has been added to the "Phenotypic differences" subsection in the R Markdown file. This includes a code chunk to simulate an example plot of how muscle mass changes over time in response to resistance *versus* endurance training, as well as a sentence summary of the final amount of muscle mass change induced by each exercise type based on the simulated data. 
